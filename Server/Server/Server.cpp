...
DWORD WINAPI serverReceive(LPVOID lpParam) { //Получение данных от клиента
	...
	while (true) { //Цикл работы сервера
		...
		cout << "Client: " << decoded_data << endl; //Иначе вывести сообщение от клиента из буфера
...
DWORD WINAPI serverSend(LPVOID lpParam) { //Отправка данных клиенту
	...
	while (true) {
		...
		if (send(client, encoded_data.c_str(), sizeof(encoded_data), 0) == SOCKET_ERROR) {
...
int main() 
{
	...
	{ //Если не удалось получить запрос
		cout << "Listen function failed with error:" << WSAGetLastError() << endl;
		return -1;
	}
	cout << "Waiting for abonent..." << endl;
